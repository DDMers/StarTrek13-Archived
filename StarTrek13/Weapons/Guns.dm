...
/obj/item/gun/energy/phaser
	name = "WIP"
	desc = "This is a work in progress and is going to be a few days at least before its in a state good enough to replace current phasers \
	...
	ammo_type = list(/obj/item/ammo_casing/energy/phaser/hitscan)
	cell_type = /obj/item/stock_parts/cell/phaser
	canMouseDown = TRUE
	pin = null
	var/aiming = FALSE
	var/aiming_time = 12
...
/obj/item/gun/energy/phaser/debug
	delay = 0
...
/obj/item/gun/energy/phaser/equipped(mob/user)
	set_user(user)
	. = ..()

/obj/item/gun/energy/phaser/pickup(mob/user)
	set_user(user)
	. = ..()

/obj/item/gun/energy/phaser/dropped(mob/user)
	set_user()
	. = ..()

/obj/item/gun/energy/phaser/ui_action_click(owner, action)
...
/obj/item/gun/energy/phaser/proc/smooth_zooming(delay_override = null)
...
/obj/item/gun/energy/phaser/proc/set_autozoom_pixel_offsets_immediate(current_angle)
...
/obj/item/gun/energy/phaser/proc/handle_zooming()
...
/obj/item/gun/energy/phaser/proc/start_zooming()
	if(zoom_lock == ZOOM_LOCK_OFF)
		return
	zooming = TRUE

/obj/item/gun/energy/phaser/proc/stop_zooming(mob/user)
	if(zooming)
		zooming = FALSE
		reset_zooming(user)

/obj/item/gun/energy/phaser/proc/reset_zooming(mob/user)
	if(!user)
		user = current_user
	if(!user || !user.client)
		return FALSE
...
/obj/item/gun/energy/phaser/update_icon()
	cut_overlays()
...
/obj/item/gun/energy/phaser/proc/update_slowdown()
	if(aiming)
		slowdown = scoped_slow
	else
		slowdown = initial(slowdown)

/obj/item/gun/energy/phaser/Initialize()
	. = ..()
	fire_delay = delay
...
/obj/item/gun/energy/phaser/Destroy()
...
/obj/item/gun/energy/phaser/emp_act(severity)
	. = ..()
	if(. & EMP_PROTECT_SELF)
		return
	chambered = null
...
/obj/item/gun/energy/phaser/proc/aiming_beam(force_update = FALSE)
	var/diff = abs(aiming_lastangle - lastangle)
	check_user()
	if(diff < AIMING_BEAM_ANGLE_CHANGE_THRESHOLD && !force_update)
		return
	aiming_lastangle = lastangle
	var/obj/item/projectile/beam/phaser/hitscan/aiming_beam/P = new
	P.gun = src
...
/obj/item/gun/energy/phaser/process()
	if(!aiming)
...
/obj/item/gun/energy/phaser/proc/check_user(automatic_cleanup = TRUE)
...
/obj/item/gun/energy/phaser/proc/process_aim()
...
/obj/item/gun/energy/phaser/proc/on_mob_move()
	check_user()
	if(aiming)
...
/obj/item/gun/energy/phaser/proc/start_aiming()
...
/obj/item/gun/energy/phaser/proc/stop_aiming(mob/user)
	set waitfor = FALSE
	aiming_time_left = aiming_time
	aiming = FALSE
	QDEL_LIST(current_tracers)
...
/obj/item/gun/energy/phaser/proc/set_user(mob/user)
...
/obj/item/gun/energy/phaser/onMouseDrag(src_object, over_object, src_location, over_location, params, mob)
	if(aiming)
		process_aim()
		aiming_beam()
...
/obj/item/gun/energy/phaser/onMouseDown(object, location, params, mob/mob)
	if(istype(mob))
		set_user(mob)
...
/obj/item/gun/energy/phaser/onMouseUp(object, location, params, mob/M)
	if(istype(object, /obj/screen) && !istype(object, /obj/screen/click_catcher))
		return
	process_aim()
...
/obj/item/gun/energy/phaser/afterattack(atom/target, mob/living/user, flag, params, passthrough = FALSE)
...
/obj/item/gun/energy/phaser/proc/sync_ammo()
	for(var/obj/item/ammo_casing/energy/phaser/AC in contents)
		AC.sync_stats()

/obj/item/gun/energy/phaser/proc/delay_penalty(amount)
...
/obj/item/ammo_casing/energy/phaser
	...
	var/structure_bleed_coeff = 0.7
	var/do_pierce = TRUE
	var/obj/item/gun/energy/phaser/host

/obj/item/ammo_casing/energy/phaser/proc/sync_stats()
	var/obj/item/gun/energy/phaser/BR = loc
	if(!istype(BR))
...
/obj/item/ammo_casing/energy/phaser/ready_proj(atom/target, mob/living/user, quiet, zone_override = "")
	. = ..()
	var/obj/item/projectile/beam/phaser/hitscan/HS_BB = BB
	if(!istype(HS_BB))
		return
...
/obj/item/ammo_casing/energy/phaser/throw_proj(atom/target, turf/targloc, mob/living/user, params, spread)
	var/turf/curloc = get_turf(user)
	if(!istype(curloc) || !BB)
		return FALSE
	var/obj/item/gun/energy/phaser/gun = loc
	if(!targloc && gun)
...
/obj/item/ammo_casing/energy/phaser/hitscan
	projectile_type = /obj/item/projectile/beam/phaser/hitscan
	select_name = "beam"
	e_cost = 5000
	fire_sound = 'sound/weapons/beam_sniper.ogg'

/obj/item/projectile/beam/phaser
	name = "particle beam"
	icon = null
	...
	range = 150
	jitter = 10
	var/obj/item/gun/energy/phaser/gun
...
/obj/item/projectile/beam/phaser/proc/AOE(turf/epicenter)
	set waitfor = FALSE
	if(!epicenter)
		return
...
/obj/item/projectile/beam/phaser/proc/check_pierce(atom/target)
	if(!do_pierce)
		return FALSE
...
/obj/item/projectile/beam/phaser/proc/get_damage_coeff(atom/target)
...
/obj/item/projectile/beam/phaser/proc/handle_impact(atom/target)
	if(isobj(target))
		var/obj/O = target
...
/obj/item/projectile/beam/phaser/proc/handle_hit(atom/target)
	set waitfor = FALSE
...
/obj/item/projectile/beam/phaser/Collide(atom/target)
...
/obj/item/projectile/beam/phaser/on_hit(atom/target, blocked = FALSE)
	if(!QDELETED(target))
		cached = get_turf(target)
	handle_hit(target)
	. = ..()

/obj/item/projectile/beam/phaser/hitscan
	icon_state = ""
	hitscan = TRUE
	tracer_type = /obj/effect/projectile/tracer/tracer/phaser
	var/constant_tracer = FALSE

/obj/item/projectile/beam/phaser/hitscan/generate_hitscan_tracers(cleanup = TRUE, duration = 5, impacting = TRUE, highlander)
	set waitfor = FALSE
...
/obj/item/projectile/beam/phaser/hitscan/aiming_beam
...
/obj/item/projectile/beam/phaser/hitscan/aiming_beam/prehit(atom/target)
	qdel(src)
	return FALSE

/obj/item/projectile/beam/phaser/hitscan/aiming_beam/on_hit()
	qdel(src)
	return FALSE

/obj/item/stock_parts/cell/phaser
	name = "beam rifle capacitor"
	desc = "A high powered capacitor that can provide huge amounts of energy in an instant."
	maxcharge = 50000
	chargerate = 5000	//Extremely energy intensive

/obj/item/stock_parts/cell/phaser/corrupt()
	return

/obj/effect/projectile/tracer/tracer/phaser
