	...
	if(CONFIG_GET(flag/log_access))
		for(var/I in GLOB.clients)
			...
			if(C.key && (C.key != key) )
				...
				if(matches)
					if(C)
						message_admins("<font color='red'><B>Notice: </B><font color='#7289da'>[key_name_admin(src)] has the same [matches] as [key_name_admin(C)].</font>")
						log_access("Notice: [key_name(src)] has the same [matches] as [key_name(C)].")
					else
						message_admins("<font color='red'><B>Notice: </B><font color='#7289da'>[key_name_admin(src)] has the same [matches] as [key_name_admin(C)] (no longer logged in). </font>")
